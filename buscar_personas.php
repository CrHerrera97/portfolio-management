...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {                
        ...
        $html .= '<div><a class="suggest-element2" data="'.utf8_encode($row['id']).'" id="'.$row['id'].''.utf8_encode($row['nombreComp']).'" nombreComp="'.$row['nombreComp'].'">'.utf8_encode($row['nombreComp']).'</a></div>';
    }
}
echo $html;
